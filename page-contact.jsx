        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          {channels.map((c, i) => (
            <a key={i} style={{
              ...
              <div>
                <div style={{
                  fontFamily: sansFF, fontSize: 11, letterSpacing: 0.5,
                  textTransform: 'uppercase', color: SITE.muted, marginBottom: 4,
                }}>{c.l}</div>
  ...
  return (
    <section style={{ background: SITE.bg, padding: '20px 32px 88px' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.35fr 1fr', gap: 28 }}>
        {/* form */}
        <form style={{ background: SITE.surface, borderRadius: 24, padding: 36, border: `1px solid ${SITE.line}` }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {topics.map((t, i) => (
                <span key={i} style={{
                  ...
                  border: `1px solid ${i === 0 ? SITE.primary : SITE.lineStrong}`,
                  background: i === 0 ? SITE.primaryTint : '#fff',
          ...
          <FormField label="Wits email">
            <Input value="2245319@students.wits.ac.za" />
          </FormField>

          <FormField label="Tell us what's going on">
            <Textarea value="Hi — I had a tutoring session booked yesterday at 14:00 with Thabo K., but he didn't show up and I didn't get the Zoom link. I'm happy to rebook, but I'd like to know what happened. Thanks!" />
          </FormField>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: SITE.ink2 }}>
              <span style={{ width: 16, height: 16, borderRadius: 4, border: `1.5px solid ${SITE.lineStrong}`,
                display: 'grid', placeItems: 'center', background: SITE.primary, borderColor: SITE.primary, color: '#fff' }}>
                <SIcon name="check" size={11} stroke={2.4} />
              </span>
              Email me a copy of this message
            </label>
            <CTA size="lg" variant="dark" icon="arrowRight">Send message</CTA>
          </div>
        </form>
...
function FormField({ label, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontFamily: sansFF, fontSize: 12.5, fontWeight: 500, color: SITE.ink2,
...
function Input({ value }) {
  return (
    <div style={{
      ...
      fontSize: 14, color: SITE.ink, fontFamily: sansFF,
    }}>{value}</div>
  );
}
function Textarea({ value }) {
  return (
    <div style={{
      width: '100%', padding: '14px 16px', borderRadius: 12, minHeight: 110,
      border: `1px solid ${SITE.lineStrong}`, background: '#fff',
      fontSize: 14, color: SITE.ink, lineHeight: 1.55, fontFamily: sansFF,
    }}>{value}</div>
  );
}
  ...
  return (
    <div style={{ background: SITE.surface, borderRadius: 24, border: `1px solid ${SITE.line}`, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 22 }}>
        <div style={{ fontFamily: dispFF, fontSize: 22, color: SITE.ink, marginBottom: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <CTA size="sm" variant="ghost" icon="arrowUpRight">Open in Maps</CTA>
...
// abstract street/grid map placeholder — not a real map
function MapDoodle() {
  return (
    ...
    <div style={{ background: SITE.primaryInk, color: '#fff', borderRadius: 24, padding: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <span style={{
          ...
          background: 'rgba(110,140,70,0.25)', color: '#A5C078',
          display: 'inline-flex', alignItems: 'center', gap: 6,
        }}>
          <span style={{ width: 6, height: 6, background: '#A5C078', borderRadius: '50%' }} /> Open now
        </span>
      </div>
  ...
  return (
    <div style={{ background: SITE.surface, borderRadius: 24, border: `1px solid ${SITE.line}`, padding: 22 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div style={{
          width: 34, height: 34, borderRadius: 8, background: SITE.accentSoft, color: SITE.accent,
          display: 'grid', placeItems: 'center',
        }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {items.map((t, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10,
            fontSize: 13, color: SITE.ink2 }}>
            <span style={{
              width: 18, height: 18, borderRadius: '50%', background: SITE.primaryTint, color: SITE.primary,
      ...
      <a style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 16,
        fontSize: 13, color: SITE.primary, fontWeight: 500, cursor: 'pointer', textDecoration: 'none' }}>
    ...
    <section style={{ background: SITE.paper, padding: '88px 32px', borderTop: `1px solid ${SITE.line}` }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 64 }}>
        <div>
          ...
          <h2 style={{
            fontFamily: dispFF, fontWeight: 400, fontSize: 40, lineHeight: 1.1,
            letterSpacing: '-0.02em', margin: '14px 0 12px',
          }}>
            ...
            <details key={i} open={i === 0} style={{
              ...
              <summary style={{
                ...
                <span style={{
                  ...
                  display: 'grid', placeItems: 'center', color: SITE.ink,
                  transform: i === 0 ? 'rotate(45deg)' : 'none', transition: 'transform .2s',
                }}>
